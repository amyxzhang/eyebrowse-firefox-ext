...
function addProxyParam(url, cookies, headers){
    ...
    for (var key in cookies){
        params += sprintf("&%s=%s", key, cookies[key])
    }
...
$ = {
    ajax: function (params) {
        ...
        console.log('REQ', JSON.stringify(params.content))
...
//var proxyUrl = "http://eyebrowse-proxy.herokuapp.com"
var activeItem;
var requestMap = {
    'GET' : 'get',
    'POST' : 'post',
...
function openItem(url, favIconUrl, title, event_type) {
    console.log("OPEN ITEM", JSON.stringify(arguments))
    var timeCheck = checkTimeDelta();
    //if its not in the whitelist lets check that the user has it
    console.log("TIMECHECK", JSON.stringify(timeCheck))
    console.log("SET CHECK", user.inWhitelist(url), user.inBlackList(url))
    if (!user.inWhitelist(url) && !user.inBlackList(url)) {

        timeCheck.allow = false; // we need to wait for prompt callback
        // chrome.tabs.sendMessage({"action": "prompt"},function(res){
        //         if (res != undefined && res.prompRes == 'allow') {
        //             finishOpen(url, favIconUrl, title, event_type);
        //         }
        //     });
...
function finishOpen(url, favIconUrl, title, event_type, time) {
    console.log("FINISH OPEN", JSON.stringify(arguments))
...
function closeItem(url, event_type, time) {
    console.log("CLOSE ITEM", JSON.stringify(arguments))
    console.log("LOCAL HISTORY", JSON.stringify(local_history))
...
function dumpData() {
    ...
    for(var index in local_history) {
        ...
        console.log("DUMP", "index", index, "history", JSON.stringify(local_history), 'item', JSON.stringify(item))
...
function getApiURL(resource, id, params) {
    ...
    console.log("API BASE", apiBase, id)
    if (id != null) {
        apiBase += '/' + id;
    } 
    return apiBase
...
function userChange(data) {
    // console.log("cs handler", JSON.stringify(data))
    var func = data.func;
    var args = data.args;
    user[func].apply(undefined, Array.prototype.slice.call(args));
    // console.log("cs handler", JSON.stringify(user))
}
