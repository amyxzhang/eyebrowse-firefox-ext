...
function openItem(url, favIconUrl, title, event_type) {
    ...
    if (!user.inWhitelist(url) && !user.inBlackList(url)) {
        ...
        // console.log("SETTING PAGE MOD")
        // var mod = setPageMod(function(res){
        //     if (res != undefined && res.prompRes == 'allow') {
        //         finishOpen(url, favIconUrl, title, event_type);
        //     }
        //     // mod.destroy(); //destroy page mod.
        // });
...
function handleFilterListMsg(message) {
    console.log("FILTERLIST MSG:", JSON.stringify(message))
...
function setPageMod(callback) {
    ...
    var mod = pageMod.PageMod({
        ...
        contentScriptFile:[
            ...
            data.url("libs/url-min.js"),
            data.url("js/idle-detection.js"),
            data.url("js/prompt.js"),
        ],
        onAttach: function(worker, callback){
            console.log("WORKER")
            promptWorker(worker, callback);
        },
    });
    return mod
}

function promptWorker(worker, callback) {
    worker.port.emit("prompt");
    worker.port.on("filterlist", handleFilterListMsg);
}
