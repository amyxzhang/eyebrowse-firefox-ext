...
function openItem(url, favIconUrl, title, event_type) {
    ...
    if (!user.inWhitelist(url) && !user.inBlackList(url)) {
        ...
        destroyMod();//destory old page mod.
        mod = setPageMod({
            "url" : url,
...
*/
function setPageMod(args) {
    console.log("SETTING MOD", args.url)
    var m = pageMod.PageMod({
        include: args.url,
        contentScriptFile:[
            data.url("libs/jquery-1.8.2.js"),
            data.url("libs/jquery.idle.js"),
            data.url("libs/url-in.js"),
            data.url("js/idle-detection.js"),
            data.url("js/prompt.js"),
        ],
        attachTo: ["top", "existing"],
        onAttach: function(worker, args) {
            console.log("attach prompt")
            worker.port.emit("prompt", {
                "baseUrl" : baseUrl,
            });
            
            worker.port.on("filterlist", function(message) {
                console.log("FILTERLISTMESSAGE", JSON.stringify(arguments))
                destroyMod();
                handleFilterListMsg(message);
                if (message.type === 'whitelist') {
                    finishOpen(args.tabId, args.url, args.favIconUrl, args.title, args.event_type);
                }
            });
        },
    });
