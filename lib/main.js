...
function ajaxWrapper(params){
    params.url = addProxyParam(params.url, user.getAllCookies());
    return params
}
...
$ = {
    ajax: function (params) {
        ...
        Request(params)[requestMap[params.type]]();
    }           
};
...
var User = Backbone.Model.extend({
    ...
    setCookie : function(key, value) {
        ...
        cookies[key] = value;
        console.log('set func', JSON.stringify(cookies), key, value)
        this.set({ 
            'cookies': cookies,
        });
    },
    ...
    inSet : function(setType, url) {
        ...
        return (set.where({'url' : host}).length || set.where({"url" : scheme}).length || set.where(url).length)
    },
